...
AUTOMATION_OUT = ROOT / "docs" / "estate" / "automation.md"
VISUAL_OUT = ROOT / "docs" / "estate" / "visualizations.md"
...
def render_workloads(categories: Dict[str, List[Dict]]) -> str:
    ...
    for category in sorted(categories.keys()):
        ...
        lines.append("")
        lines.append("| Workload | Repository | Environments | Subscriptions |")
        lines.append("| --- | --- | --- | --- |")
        for workload in sorted(categories[category], key=lambda w: w["name"]):
            ...
            lines.append(f"| {workload['name']} | {repo_link} | {envs} | {subs} |")
        lines.append("")
    lines.append("---")
...
def render_automation(last_run: datetime) -> str:
    iso = last_run.isoformat().replace("+00:00", "Z")
    lines = [
        "# Automation Plan",
        "",
...
def render_visualizations(repos: List[Tuple[str, Optional[str], Optional[str]]]) -> str:
    badges = [release for _, release, _ in repos if release]
    lines = [
        "# Visualizations",
        "",
        "## Badge Wall",
    ...
    if badges:
        ...
        lines.append("No release badges detected.")
        lines.append("")
    lines.append("## Dependency Graph (concept)")
    lines.append("")
    lines.append("```mermaid")
    lines.append("graph TD")
    lines.append("  PlatformWorkloads[platform-workloads] --> Workloads[Estate workloads]")
    lines.append("```")
    lines.append("")
    lines.append("---")
    lines.append("Generated by scripts/estate-sync/estate_sync.py")
...
def main() -> None:
    ...
    AUTOMATION_OUT.write_text(render_automation(datetime.now(timezone.utc)), encoding="utf-8")
    VISUAL_OUT.write_text(render_visualizations(repo_badges), encoding="utf-8")
